...
**Github Repository: https://github.com/CGutt-hub/EmotiView**
...
*   **`EV_proposal.pdf`**: This document provides an in-depth description of the research background, aims, hypotheses, detailed methodology, analysis plan, and project timeline.
*   **`EV_analysis/`**: Contains the Python-based analysis pipeline developed to implement the methods described in the proposal.
*   **`EV_results/`**: Directory where the pipeline saves processed data, analysis metrics, and plots.

## Analysis Pipeline

To operationalize the methods outlined in the research proposal, a modular Python-based analysis pipeline has been developed and is located in the `EV_analysis/` directory. This pipeline is designed to:
...
While the detailed workings of the pipeline are contained within the `EV_analysis/` subfolder (including its own configuration in `EV_analysis/config.py`), its design and implementation are directly guided by the analytical requirements of this research project. Users interested in the technical implementation of the analysis should refer to the scripts within that directory.

## Project Status

Data collection and Pipeline Refinement.
