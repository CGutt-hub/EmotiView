...
workflow {
	...
	// Terminal processes are all plotters - watchdog monitors trace file for these to complete
	def terminal_processes = "panas_plotter,bisbas_plotter,sam_plotter,be7_plotter,ea11_plotter,hrv_plotter,eda_plotter,psd_plotter,fai_plotter,fnirs_plotter,fnirs_asym_plotter,fai_combined_plotter,hrv_rel_plotter,eda_rel_plotter,psd_rel_plotter,fai_rel_plotter,fnirs_rel_plotter,fnirs_asym_rel_plotter,fai_combined_rel_plotter"
	workflow_wrapper(params.input_dir, params.output_dir, params.participant_pattern, terminal_processes)
	
	// Get synchronized participant context and split into separate channels
	participant_context = workflow_wrapper.out.participant_context
	...
	// Use type:EEG to find first EEG stream regardless of position
	...
	// Use type:NIRS to find first NIRS stream regardless of position
	...
	// Channel selection: skip frame (idx 0), take pairs at stride 2 to get 2 wavelengths from 4
	...
	// PANAS and BIS-BAS are multi-item questionnaires that produce multiple dimensions/subscales
    ...
    // Concatenate multi-condition data (condition-based questionnaires)
    ...
    // Concatenate multi-condition data (condition-based physiological)
	...
	// Args: pairs mode band y_lim y_label suffix
	...
	// fNIRS: ROI analysis (grouped by PFC regions using NIRx channel patterns)
	// Per-epoch baseline correction: subtract mean of first N seconds from each epoch before averaging
	fnirs_analyzed = group_analyzer(params.python_exe, params.group_analyzer_script, fnirs_epoched, "${params.fnirs_rois} ${params.fnirs_y_lim} ROI 'Hb Change (μM)' hbc ${params.fnirs_epoch_baseline_sec} ${params.fnirs_chromophore_mode}")
	...
	// Combined FAI: Merge PSD FAI and fNIRS FAI per participant (uses generic merging_processor --plot mode)
	fai_input = participant_id
		.join(psd_fai_concat.map { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] })
		.join(fnirs_fai_concat.map { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] })
	fai = fai_merger(params.python_exe, params.merging_processor_script,
		fai_input.map { pid, eeg, fnirs -> [eeg, fnirs] },
		"--plot PSD,fNIRS fai")
	
	// Step 5b: Relative normalization (baseline-normalized to NEU condition, NEU excluded from final output)
	// Uses existing concatenated data (all 3 conditions) - relative_analyzer will subtract NEU baseline
	// and exclude NEU from the output (showing only POS and NEG relative to NEU)
	...
	fai_rel = fai_combined_relative_analyzer(params.python_exe, params.relative_analyzer_script, fai, "NEU ${params.fai_combined_rel_y_lim}")

	// Step 6: Plotting - join data with context by participant ID to ensure correct matching
	// PID extraction: inline in each map to avoid Nextflow closure scope issues
	
	// Multi-item questionnaire plots (PANAS/BIS-BAS use raw multi-item data)
	...
	// Condition-based questionnaire plots (SAM/BE7/EA11 use concatenated data)
	...
	// Physiological plots (all concatenated data from concatenating_processor)
	...
	fai_with_ctx = fai.map { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] }.join(participant_context)
	fai_plot = fai_combined_plotter(params.python_exe, params.plotter_script, 
		fai_with_ctx.map { pid, f, folder -> f },
		fai_with_ctx.map { pid, f, folder -> "${workflow.launchDir}/${folder} ${pid}_fai" })

	// Relative-normalized physiological plots (baseline-subtracted relative to NEU)
	...
	fai_rel_with_ctx = fai_rel.map { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] }.join(participant_context)
	fai_rel_plot = fai_combined_rel_plotter(params.python_exe, params.plotter_script, 
		fai_rel_with_ctx.map { pid, f, folder -> f },
		fai_rel_with_ctx.map { pid, f, folder -> "${workflow.launchDir}/${folder} ${pid}_fai_rel" })
	}