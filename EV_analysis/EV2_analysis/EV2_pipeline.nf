...
include {
    ...
    eda_l2_lmm_analyzer; hrv_l2_lmm_analyzer; fai_l2_lmm_analyzer;
    ...
    eeg_frontal_l2_lmm_analyzer; eeg_parietal_l2_lmm_analyzer;
    l2_lmm_consolidator;
    wp2_l2_correl;
    wp3_l2_lmm;
    wp4_l2_lmm;
...
// All output files are prefixed EV2_NN_* so split('_')[0..1].join('_') = "EV2_NN"
...
workflow {
    ...
    dat_triggers = deap_bootstrap(params.python_exe, params.ev2_dataset_setup_script,
        kaggle_config, "s*.dat EV2 dat_path 32")
        .flatten()
        ...
        .filter { f -> f.name ==~ /EV2_\d{2}_dat_path\.parquet/ }

    // Normalise participant ID from trigger filename: EV2_01_dat.parquet → "EV2_01"
    ...
    // Step 3b. SAM-equivalent: DEAP affective ratings (valence/arousal/dominance/liking)
    //   Reshape labels wide-to-long so each rating dimension becomes a condition,
    //   then bootstrap per dimension for a mean ± CI bar chart per participant.
    sam_long = sam_pivot(params.python_exe, params.pivot_processor_script,
        labels, "wide_to_long trial_id None valence,arousal,dominance,liking")
    sam_result = sam_bootstrap(params.python_exe, params.bootstrap_analyzer_script,
        sam_long, "variable trial_id value ${params.bootstrap_n_boot} ${params.bootstrap_ci_method} ${params.bootstrap_alpha} None 'Rating (1-9)' result terminal")

    // Step 4: EEG analysis
    // 4a. Full-spectrum per-condition (spectrum_analyzer bins trials by valence/arousal
    //     when a labels file is supplied as the second input, producing a per-condition
    //     line plot directly comparable to the EV1 spectrum result).
    eeg_spectrum_inputs = participant_id
    ...
    eeg_spectrum = eeg_spectrum_analyzer(params.python_exe, params.spectrum_analyzer_script,
        eeg_spectrum_inputs,
        "None None ${params.eeg_max_freq} ${params.ev2_valence_threshold} ${params.ev2_arousal_threshold} ${params.ev2_neutral_band}")
    ...
    eeg_alpha_psd = eeg_fai_psd_analyzer(params.python_exe, params.psd_analyzer_script,
        eeg_epochs, "{'alpha': [8, 13]} None None result")

    // 4b. FAI per-trial flat table (intermediate — visualised downstream as fai_bootstrap bar chart)
    ...
    eeg_roi_psd = eeg_roi_psd_analyzer(params.python_exe, params.psd_analyzer_script,
        eeg_windowed, "${params.bands_config} None ${params.eeg_rois} ${params.psd_y_lim} result")
    ...
    fai_correlation = fai_correlation_analyzer(params.python_exe,
        ...
        "--correlate-ratings valence,arousal None spearman terminal result")
    ...
    fai_binned = fai_label_binner(params.python_exe, params.ev2_label_binner_script,
        fai_bin_inputs,
        "${params.ev2_valence_threshold} ${params.ev2_arousal_threshold} ${params.fai_column} ${params.ev2_neutral_band} terminal")
    ...
    // LMM: DV ~ C(condition) + (1 | participant_id)
    // Replaces the pseudoreplicating one-way ANOVA with a proper Linear Mixed Model
    // that accounts for between-participant variance via a random intercept.
    eda_l2_lmm = eda_l2_lmm_analyzer(params.python_exe, params.lmm_analyzer_script,
        eda_binned.collect(),
        "auto condition group_log table terminal")
    hrv_l2_lmm = hrv_l2_lmm_analyzer(params.python_exe, params.lmm_analyzer_script,
        hrv_binned.collect(),
        "auto condition group_log table terminal")
    fai_l2_lmm = fai_l2_lmm_analyzer(params.python_exe, params.lmm_analyzer_script,
        fai_binned.collect(),
        "auto condition group_log table terminal")

    eeg_frontal_l2_lmm = eeg_frontal_l2_lmm_analyzer(params.python_exe, params.lmm_analyzer_script,
        eeg_frontal_binned.collect(),
        "auto condition group_log table terminal")
    eeg_parietal_l2_lmm = eeg_parietal_l2_lmm_analyzer(params.python_exe, params.lmm_analyzer_script,
        eeg_parietal_binned.collect(),
        "auto condition group_log table terminal")
    l2_lmm_outputs = eda_l2_lmm.mix(hrv_l2_lmm).mix(fai_l2_lmm).mix(eeg_frontal_l2_lmm).mix(eeg_parietal_l2_lmm)
    l2_lmm_outputs_valid = l2_lmm_outputs.filter { !it.baseName.contains('sentinel') }
    l2_lmm_summary = l2_lmm_consolidator(params.python_exe, params.l2_lmm_consolidator_script,
        l2_lmm_outputs_valid.collect(),
        "--consolidate-l2 --modality-map ${params.l2_anova_modality_map} l2_lmm_summary group_log table terminal")
    l2_outputs = l2_lmm_outputs.mix(l2_lmm_summary)

    // Step 11: L2 WP2 — Cross-modal consistency: aggregate per-participant correl
    //   heatmaps into a group-level heatmap (mean r, SE, t-test vs 0).
    //   Uses correl_analyzer.py --consolidate-l2 (already handles this aggregation pattern).
    wp2_l2 = wp2_l2_correl(params.python_exe, params.correl_analyzer_script,
        multimodal_correl_result.collect(),
        "--consolidate-l2 wp2_l2 group_log result terminal")
    l2_outputs = l2_outputs.mix(wp2_l2)

    // Step 12: L2 WP3 — Vagal tone proxy × emotional reactivity (N=32)
    //   Contextual LMM: EDA_ij ~ HRV_within_ij + HRV_between_i + C(condition) + (1|participant)
    //   beta_between answers the between-person question: do high-HRV participants show
    //   higher EDA reactivity? Random intercept absorbs individual baselines.
    //   beta_within answers the bonus within-person question: trial-level coupling.
    wp3_l2 = wp3_l2_lmm(params.python_exe, params.lmm_analyzer_script,
        hrv_binned.mix(eda_binned).collect(),
        "--contextual eda hrv condition wp3_contextual_lmm group_log result terminal")
    l2_outputs = l2_outputs.mix(wp3_l2)

    // Step 13: L2 WP4 — FAI lateralization × autonomic reactivity (N=32)
    //   Two contextual LMMs: EDA ~ FAI (within+between) + condition + (1|participant)
    //                    and HRV ~ FAI (within+between) + condition + (1|participant)
    //   Combined into one call: DV=auto picks both EDA and HRV, covariate=fai.
    wp4_l2 = wp4_l2_lmm(params.python_exe, params.lmm_analyzer_script,
        fai_binned.mix(eda_binned).mix(hrv_binned).collect(),
        "--contextual auto fai condition wp4_contextual_lmm group_log result terminal")
    l2_outputs = l2_outputs.mix(wp4_l2)
    
    // LGC-RCT Leave-One-Subject-Out (LOSO) cross-validation analyzer
    lgcrct_l2 = lgcrct_l2_analyzer(params.python_exe, params.lgcrct_loso_analyzer_script,
        eeg_epochs.collect(),
        "--mode ${params.lgcrct_mode} --targets ${params.lgcrct_targets} --sfreq ${params.ev2_sfreq} --window-sec ${params.lgcrct_window_sec} --step-sec ${params.lgcrct_step_sec} --band ${params.lgcrct_band} --threshold ${params.lgcrct_threshold} --use-lgc ${params.lgcrct_use_lgc} --half-window ${params.lgcrct_half_window} --cov-estimator ${params.lgcrct_cov_estimator} --lgc-mean ${params.lgcrct_lgc_mean} group_log table terminal")
