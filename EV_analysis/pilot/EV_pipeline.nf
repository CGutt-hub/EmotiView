...
workflow {
	...
	// Step 3b: Phase spectrum — per-ROI circular mean phase, computed directly from epoched EEG
	eeg_phase_analyzed = eeg_phase_analyzer(params.python_exe, params.phase_analyzer_script, eeg_epoched,
		"None ${params.eeg_rois} 45")
	eeg_phase1_raw = eeg_phase1_file_finder(params.python_exe, params.file_finder_script, eeg_phase_analyzed, "*phase1.parquet eegphase")
	eeg_phase2_raw = eeg_phase2_file_finder(params.python_exe, params.file_finder_script, eeg_phase_analyzed, "*phase2.parquet eegphase")
	eeg_phase3_raw = eeg_phase3_file_finder(params.python_exe, params.file_finder_script, eeg_phase_analyzed, "*phase3.parquet eegphase")
	eeg_phase_files = participant_id
		.join(eeg_phase1_raw.map { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] })
		.join(eeg_phase2_raw.map { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] })
		.join(eeg_phase3_raw.map { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] })
		.map { pid, f1, f2, f3 -> [f1, f2, f3] }
	eeg_phase_concat = eeg_phase_concatenating_processor(params.python_exe, params.concatenating_processor_script, eeg_phase_files, "eeg_phase terminal")
	...
	eeg_phase_result = eeg_phase_result_collector(params.python_exe, params.result_collector_script, eeg_phase_concat, "eeg_phase result terminal")
...
// Step 5c: ANOVA per modality — one-way ANOVA across conditions per DV
    ...
    // Step 5d: Combine all 6 epoch tables (with injected 'source' label) → single ANOVA
    ...
    anova_epochs_concat = anova_merger(params.python_exe, params.concatenating_processor_script, anova_epoch_files, "labels=EDA,HRV,EEG_Frontal,EEG_Parietal,fNIRS,FAI anova_epochs terminal")
    anova_combined = anova_combined_analyzer(params.python_exe, params.anova_analyzer_script, anova_epochs_concat, "auto condition true None source terminal")
    ...
    // Step 5e: Multimodal cross-correlation heatmap (EEG frontal + EDA + HRV joined on condition-level means)
    multimodal_files = participant_id
            .join(eeg_frontal_epochs.map { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] })
            .join(eda_amp_concat.map    { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] })
            .join(hrv_interv_concat.map { f -> [f.baseName.toString().split('_')[0..1].join('_'), f] })
            .map { pid, eeg, eda, hrv -> [eeg, eda, hrv] }
    multimodal_joined = multimodal_epoch_processor(params.python_exe, params.multimodal_epoch_processor_script, multimodal_files, "terminal")
    multimodal_correl = multimodal_correl_analyzer(params.python_exe, params.correl_analyzer_script, multimodal_joined, "None terminal")
    ...
    finalize_participant(
            panas_result.mix(bisbas_result, sam_result, be7_result, ea11_result,
                             ...
                             eeg_fai_result, fnirs_result, fnirs_posneg_result, eeg_phase_result,
