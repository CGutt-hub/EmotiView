...
EDA_CH = 36   # GSR channel index
BVP_CH = 37   # BVP channel index
...
def ingest(dat_path: str, strip_baseline: bool = True, sfreq: int = 128) -> str:
    ...
    base           = f"DEAP_{participant_num}"                         # e.g. "DEAP_01"
    ...
    pl.concat(frames).write_parquet(eeg_path, compression='snappy')
    ...
    pl.concat(frames).write_parquet(eda_path, compression='snappy')
    ...
    pl.concat(frames).write_parquet(bvp_path, compression='snappy')
    ...
    pl.concat(baseline_frames).write_parquet(bvp_baseline_path, compression='snappy')
    ...
    }).write_parquet(labels_path, compression='snappy')
