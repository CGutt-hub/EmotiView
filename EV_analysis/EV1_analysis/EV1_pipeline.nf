...
// Import all modules from EV1_modules.nf
...
include { 
    ...
    fnirs_raw_concatenating_processor; fai1_epoch_analyzer; fai2_epoch_analyzer; fai3_epoch_analyzer; fai_epoch_concatenating_processor; anova_merger; anova_combined_analyzer; anova_combined_l2_lmm_analyzer; multimodal_correl_analyzer;
	...
	lgcrct_l2_analyzer;
    sam_eeg_valence_l1_lmm_analyzer; sam_eeg_arousal_l1_lmm_analyzer;
    sam_hrv_valence_l1_lmm_analyzer; sam_hrv_arousal_l1_lmm_analyzer;
    sam_l1_lmm_result_collector;
} from './EV1_modules.nf'
...
workflow {
	...
	txt_data = txt_reader(params.python_exe, params.txt_reader_script, participant_id.map { id -> "${workflow.launchDir}/${params.input_dir}/${id}/${id}.txt" }, "${params.txt_encoding}")

	// xdf_reader produces per-stream fif/parquet files
	xdf_data = xdf_reader(params.python_exe, params.xdf_reader_script, participant_id.map { id -> "${workflow.launchDir}/${params.input_dir}/${id}/${id}.xdf" }, "")
	...
	eeg_roi_psd = eeg_roi_psd1_analyzer(params.python_exe, params.psd_analyzer_script, eeg_windowed, "${params.bands_config} None ${params.eeg_rois} ${params.psd_y_lim}")
	...
	eeg_spectrum_analyzed 	= eeg_spectrum_analyzer(params.python_exe, params.spectrum_analyzer_script, eeg_epoched, "None ${params.eeg_rois} 45")
	...
	eeg_psd1_clean = eeg_psd1_outlier_processor(params.python_exe, params.rejection_processor_script, eeg_psd1_epochs, "${params.eeg_band_list} zscore ${params.psd_outlier_threshold}")
	eeg_psd2_clean = eeg_psd2_outlier_processor(params.python_exe, params.rejection_processor_script, eeg_psd2_epochs, "${params.eeg_band_list} zscore ${params.psd_outlier_threshold}")
	eeg_psd3_clean = eeg_psd3_outlier_processor(params.python_exe, params.rejection_processor_script, eeg_psd3_epochs, "${params.eeg_band_list} zscore ${params.psd_outlier_threshold}")
	...
	eeg_psd_clean_files = participant_id
							...
							.map { pid, f1, f2, f3 -> [f1, f2, f3] }
	eeg_psd_epochs 		= eeg_psd_concatenating_processor(params.python_exe, params.concatenating_processor_script, eeg_psd_clean_files, "eeg_psd_epochs")
	...
	eeg_alpha_bootstrapped 	= eeg_alpha_bootstrap_analyzer(params.python_exe, params.bootstrap_analyzer_script, eeg_frontal_epochs, "condition epoch_id alpha ${params.bootstrap_n_boot} ${params.bootstrap_ci_method} ${params.bootstrap_alpha} ${params.psd_y_lim} 'Alpha Power (µV²/Hz)' terminal")
	eeg_beta_bootstrapped 	= eeg_beta_bootstrap_analyzer(params.python_exe, params.bootstrap_analyzer_script, eeg_frontal_epochs, "condition epoch_id beta ${params.bootstrap_n_boot} ${params.bootstrap_ci_method} ${params.bootstrap_alpha} ${params.psd_y_lim} 'Beta Power (µV²/Hz)' terminal")
	eeg_theta_bootstrapped 	= eeg_theta_bootstrap_analyzer(params.python_exe, params.bootstrap_analyzer_script, eeg_frontal_epochs,	"condition epoch_id theta ${params.bootstrap_n_boot} ${params.bootstrap_ci_method} ${params.bootstrap_alpha} ${params.psd_y_lim} 'Theta Power (µV²/Hz)' terminal")

	// Step 7b: Bootstrap CIs per condition per band (Parietal ROI)
	eeg_parietal_alpha_bootstrapped = eeg_parietal_alpha_bootstrap_analyzer(params.python_exe, params.bootstrap_analyzer_script, eeg_parietal_epochs, "condition epoch_id alpha ${params.bootstrap_n_boot} ${params.bootstrap_ci_method} ${params.bootstrap_alpha} ${params.psd_y_lim} 'Alpha Power (µV²/Hz)' terminal")
	eeg_parietal_beta_bootstrapped 	= eeg_parietal_beta_bootstrap_analyzer(params.python_exe, params.bootstrap_analyzer_script, eeg_parietal_epochs, "condition epoch_id beta ${params.bootstrap_n_boot} ${params.bootstrap_ci_method} ${params.bootstrap_alpha} ${params.psd_y_lim} 'Beta Power (µV²/Hz)' terminal")
	eeg_parietal_theta_bootstrapped = eeg_parietal_theta_bootstrap_analyzer(params.python_exe, params.bootstrap_analyzer_script, eeg_parietal_epochs, "condition epoch_id theta ${params.bootstrap_n_boot} ${params.bootstrap_ci_method} ${params.bootstrap_alpha} ${params.psd_y_lim} 'Theta Power (µV²/Hz)' terminal")
	...
	eeg_psd_ols = eeg_psd_ols_processor(params.python_exe, params.ols_processor_script, eeg_psd_epochs, "eeg_psd_ols")
	...
	eeg_psd_contrasts = eeg_psd_contrast_processor(params.python_exe, params.contrast_processor_script, eeg_psd_ols, "${params.eeg_psd_contrasts} eeg_psd_contrast terminal")

	// FAI (F3–F4 frontal alpha asymmetry): separate channel-mode PSD — needs per-electrode resolution
	psd_fai_analyzed = psd_fai_analyzer(params.python_exe, params.psd_analyzer_script, eeg_windowed, "${params.bands_config} None None ${params.psd_y_lim}")
	...
	fai1_analyzed 	= fai1_analyzer(params.python_exe, params.asymmetry_analyzer_script, psd1_raw, "${params.electrode_pairs} log ${params.fai_band_name} ${params.fai_y_lim} None")
	fai2_analyzed 	= fai2_analyzer(params.python_exe, params.asymmetry_analyzer_script, psd2_raw, "${params.electrode_pairs} log ${params.fai_band_name} ${params.fai_y_lim} None")
	fai3_analyzed 	= fai3_analyzer(params.python_exe, params.asymmetry_analyzer_script, psd3_raw, "${params.electrode_pairs} log ${params.fai_band_name} ${params.fai_y_lim} None")
...
// Step 5c: ANOVA per modality — one-way ANOVA across conditions per DV
    ...
    fai1_epoch 		= fai1_epoch_analyzer(params.python_exe, params.asymmetry_analyzer_script, psd1_raw, "${params.electrode_pairs} log ${params.fai_band_name} None None true terminal")
    fai2_epoch 		= fai2_epoch_analyzer(params.python_exe, params.asymmetry_analyzer_script, psd2_raw, "${params.electrode_pairs} log ${params.fai_band_name} None None true terminal")
    fai3_epoch 		= fai3_epoch_analyzer(params.python_exe, params.asymmetry_analyzer_script, psd3_raw, "${params.electrode_pairs} log ${params.fai_band_name} None None true terminal")
    ...
    // Step 6: Finalize participants — triggers per participant when all result collectors complete.
    ...
    finalize_participant(
            panas_result.mix(bisbas_result, sam_result, be7_result, ea11_result,
                             ...
                             multimodal_correl_result, sam_prediction_result),
            17,
		participant_context
	)
