...
// Step 5c: ANOVA per modality — one-way ANOVA across conditions per DV
    ...
    // Step 5d-bis: Group-level LMM (replaces ANOVA pseudoreplication with random intercept per participant)
    l2_lmm = anova_combined_analyzer(params.python_exe, params.lmm_analyzer_script,
        anova_epochs_concat.collect(),
	...
	if (params.enable_lgcrct_branch && params.l2_analyses) {
		...
		finalize_l2(lgcrct_l2.mix(l2_lmm))
	} else if (params.l2_analyses) {
		finalize_l2(l2_lmm)
	}
